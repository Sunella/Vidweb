...
class MijovideosUpload extends JObject {
    ...
    public function process() {
        ...
        $item_id = MijoVideos::getInput()->getInt('item_id', null);

        //@TODO Check if we need to replace an existing media item

        if (empty($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
            $this->setError(JText::_('COM_MIJOVIDEOS_ERROR_UBER_PHP_UPLOAD'));
            return false;
        } else {

            //$duration = MijoVideos::get('videos')->getDuration();
            ...
            if (!empty($item_id) and $item_id > 0 and $acl->canEdit()) {
                ...
                if (strpos($row->source,'http://') === false) {
                    $files = MijoVideos::get('files')->getVideoFiles($item_id);
                    $files_model = MijoVideos::get('controller')->getModel('files');
                    foreach ($files as $dfile) {
                        $ids[] = $dfile->id;
                    }
                    $files_model->delete($ids);
                }
                JRequest::setVar('cid', $item_id, 'post');
                $row->id = $item_id;
                MijoVideos::get('controller')->updateField('videos', 'modified', $date->format('Y-m-d H:i:s'), $model);
            } else {
                $channel_id = MijoVideos::getInput()->getInt('channel_id', null);
                $channel_table = MijoVideos::getTable('MijovideosChannels');
        ...
        $location = "/media/com_mijovideos/videos/".$row->id."/orig/".$filename;
        ...
        if (JFile::upload($src, JPATH_ROOT. $location)) {
            MijoVideos::get('files')->add($row, $ext, $filename, 'orig', 200);
        } else {
            $this->setError(JText::_('COM_MIJOVIDEOS_ERROR_FILE_COULD_NOT_BE_COPIED_TO_UPLOAD_DIRECTORY'));
            return false;
        }

        // Update source field
        JRequest::setVar('cid', $row->id, 'post');
        MijoVideos::get('controller')->updateField('videos', 'source', $filename, $model);
        $row->source = $filename;

        //@TODO Check if we need to replace an existing media item

        //$this->convertToHtml5($row, $location);
...
//****************************************************************************************************************
        ...
        require_once(JPATH_MIJOVIDEOS_ADMIN . '/library/uber/ubr_ini.php');
        require_once(JPATH_MIJOVIDEOS_ADMIN . '/library/uber/ubr_lib.php');
        require_once(JPATH_MIJOVIDEOS_ADMIN . '/library/uber/ubr_finished_lib.php');
...
/////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ($_FILE_DATA as $arrayKey => $slot) {
            ...
            $tmp_name = JPATH_ROOT . '/media/com_mijovideos/videos/' . $slot->name;
            if (!isset($slot->name) || !file_exists($tmp_name)) {
                $this->setError(JText::_('COM_MIJOVIDEOS_UPLOAD_ERROR'));
            ...
            if (!$error) {
                ...
                if (!empty($item_id) and $item_id > 0 and $acl->canEdit()) {
                    ...
                    $post['product_id']     = '';
                    $post['title']          = $title;
                    $post['alias']          = JFilterOutput::stringURLSafe($title);
                    $post['introtext']      = '';
                    $post['fulltext']       = '';
                    $post['source']         = '';
                    $post['duration']       = '';
            ...
            if (in_array($ext, $types)) {
                JFolder::create(JPATH_ROOT. "/media/com_mijovideos/videos/" . $row->id . "/orig");
                if (JFile::move($src, JPATH_ROOT .$location)) {
                ...
                $this->setError(JText::_('COM_MIJOVIDEOS_ERROR_EXTENSION_NOT_ALLOWED'));
            ...
            if (substr(PHP_OS, 0, 3) != "WIN") {
                ...
                @exec("env -i " . $this->config->get('php_path', '/usr/bin/php') . " $cli convertToHtml5 " . $row->id . " " . $location . " > /dev/null 2>&1  &", $output, $error);
            }
            else {
                @exec('where php.exe', $php_path);
                // @TODO Log if throw an error
                @exec($config->get('php_path', $php_path)." $cli convertToHtml5 " . $row->id . " " . $location . " NUL", $output , $error);
            }
    ...
    public function addProcesses($item) {
        ...
        $video_size = MijoVideos::get('utility')->getVideoSize($location);
    ...
    public function remoteLink() {
        ...
        $data = MijoVideos::getInput()->getString('remote_links', array(), 'post', 'array');
        $urls = explode("\n", $data);
        ...
        foreach ($urls as $url) {
            ...
            if (empty($host)) {
                $this->setError(JText::_('COM_MIJOVIDEOS_WRONG_LINK'));
                return false;
            }

            $plugin = MijoVideos::getPlugin($host);
            ...
            if (!empty($plugin)) {
                ...
                $this->setError(JText::sprintf('COM_MIJOVIDEOS_X_PLUGIN_DISABLED_NOT_INSTALLED', ucfirst($host)));
                return false;
            }

            JTable::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_mijovideos/tables');
            $row = JTable::getInstance('MijovideosVideos', 'Table');
