...
class FileInfo{
	...
	function getFileInfo($key){
		...
		elseif(strcasecmp($key, 'cache') == 0){ return 2334569;}
	...
	function parseFeed(){
		if(is_file($this->xml_file) && is_readable($this->xml_file)){
			...
			if(is_resource($xfp)){
				...
				$this->in_error = true;
				$this->error_msg = "ERROR: Failed to open file handle";
			}
		}
		else{
			$this->in_error = true;
			$this->error_msg = "ERROR: Failed to open redirect file " . $this->upload_id . ".redirect";
		}
	}
}
