...
class MijovideosModelReports extends MijovideosModel {
        ...
        if (empty($this->_data)) {
            ...
            foreach ($rows as $row) {
                ...
                if ($row->item_type == "videos") {
                    $sql = "SELECT v.title FROM #__mijovideos_videos AS v WHERE v.id = {$row->item_id}";
                } else {
                    $sql = "SELECT c.title FROM #__mijovideos_channels AS c WHERE c.id = {$row->item_id}";
                }
    ...
    public function _buildViewQuery() {
        ...
        $this->_query = 'SELECT rs.*,r.* FROM #__mijovideos_reports AS r '
            .'LEFT JOIN #__mijovideos_report_reasons AS rs '
    ...
    public function getTotal() {
		if (empty($this->_total)) {
			$this->_total = MijoDB::loadResult("SELECT COUNT(*) FROM #__{$this->_component}_{$this->_table} AS r LEFT JOIN #__mijovideos_report_reasons AS rs ON (rs.id=r.reason_id)".$this->_buildViewWhere());
		}
    ...
    public function getReasons() {
		return MijoDB::loadObjectList('SELECT id, title FROM #__mijovideos_report_reasons WHERE language IN ('. $this->_db->Quote('*') .', '. $this->_db->Quote(JFactory::getLanguage()->getTag()) .')');
	}
