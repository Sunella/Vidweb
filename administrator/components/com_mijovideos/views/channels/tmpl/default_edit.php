    ...
    jQuery(function() {
        jQuery("#auto_video").autocomplete({
            source: function( request, response ) {
                ...
                var url = "index.php?option=com_mijovideos&view=channel&task=autocomplete&layout=submit&format=raw&tmpl=component&query="+ query;
                <?php } else { ?>
                var url = "index.php?option=com_mijovideos&view=channels&task=autocomplete&format=raw&tmpl=component&query="+ query;
                <?php } ?>
                jQuery.ajax({
                    url: url,
                    dataType: "json",
            ...
            select: function( channel, ui ) {
                ...
                <?php if (JFactory::getApplication()->isSite()) { ?>
                var url = "index.php?option=com_mijovideos&view=channel&task=createAutoFieldHtml&layout=submit&format=raw&tmpl=component&fieldid="+ ui.item.value;
                <?php } else { ?>
                var url = "index.php?option=com_mijovideos&view=channels&task=createAutoFieldHtml&format=raw&tmpl=component&fieldid="+ ui.item.value;
                <?php } ?>
...
<form action="<?php echo MijoVideos::get('utility')->getActiveUrl(); ?>" method="post" name="adminForm" enctype="multipart/form-data" id="adminForm">
    ...
    <?php echo JHtml::_('tabs.start', 'mijovideos', array('useCookie'=>1)); ?>
        ...
        <table class="admintable">
            ...
            <tr>
                <td class="key2"><?php echo JText::_('COM_MIJOVIDEOS_PICTURE_BANNER'); ?></td>
                <td class="value2">
                    ...
                    <?php if ($this->item->banner and file_exists(JPATH_ROOT.'/media/com_mijovideos/images/channels/'.$this->item->id.'/banner/thumb/'.$this->item->banner)) { ?>
                        <a href="<?php echo JUri::root().'media/com_mijovideos/images/channels/'.$this->item->id.'/banner/thumb/'.$this->item->banner; ?>" class="modal"><img src="<?php echo JUri::root().'media/com_mijovideos/images/channels/'.$this->item->id.'/banner/thumb/'.$this->item->banner; ?>" class="img_preview" /></a>
                        <input type="checkbox" name="del_thumb" value="1" /><?php echo JText::_('COM_MIJOVIDEOS_DELETE_CURRENT'); ?>
                    <?php } ?>
                </td>
            </tr>
        ...
        <table class="admintable" width="100%">
            ...
            <tr>
                <td class="key2">
                    <span class="editlinktip hasTip" title="<?php echo JText::_('COM_MIJOVIDEOS_DEFAULT_CHANNEL');?>::<?php echo JText::_('COM_MIJOVIDEOS_DEFAULT_CHANNEL_EXPLAIN'); ?>"><?php echo JText::_('COM_MIJOVIDEOS_DEFAULT_CHANNEL'); ?></span>
                </td>
                <td class="value2">
            ...
            <tr>
                <td class="key2">
                    <span class="editlinktip hasTip" title="<?php echo JText::_('COM_MIJOVIDEOS_SHARE_OTHERS');?>::<?php echo JText::_('COM_MIJOVIDEOS_SHARE_OTHERS_EXPLAIN'); ?>"><?php echo JText::_('COM_MIJOVIDEOS_SHARE_OTHERS'); ?></span>
                </td>
                <td class="value2">
            ...
            <tr>
                <td class="key2">
                    <?php echo JText::_('COM_MIJOVIDEOS_USER'); ?>
                </td>
                <td class="value2">
                    <?php echo MijoVideos::get('utility')->getUserInputbox($this->item->user_id); ?>
                </td>
            </tr>
            <?php } else { ?>
        ...
        <!-- Channel Stats -->
        <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_STATSISTICS'), 'publishing'); ?>
        <table class="admintable" width="100%">
            <tr>
                <td class="key2">
                ...
                <td class="value2">
                    ...
                    <div class="button2-left">
                        <div class="blank">
                            &nbsp;<a class="btn" id="hits" title="<?php echo JText::_('COM_MIJOVIDEOS_RESET') ?>" href="<?php echo JRoute::_('index.php?option=com_mijovideos&view=channels&task=resetstats&type=hits&id='.$this->item->id); ?>"><?php echo JText::_('COM_MIJOVIDEOS_RESET') ?></a>
                        </div>
                    </div>
                    <?php } ?>
                </td>
        ...
        <?php if($this->config->get('custom_fields')) { ?>
            <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_VIDEOS_SL_CF_TITLE'), 'sl_custom'); ?>
