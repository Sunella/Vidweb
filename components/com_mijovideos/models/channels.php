...
class MijovideosModelChannels extends MijovideosModel {
    ...
    public function _buildViewQuery() {
        ...
        $this->_query = 'SELECT c.*'
            .' FROM #__mijovideos_channels AS c'
            .' RIGHT JOIN'
            . ' #__mijovideos_videos AS v'
    ...
    public function _buildViewWhere() {
        ...
        if ($this->_mainframe->getLanguageFilter()) {
            $where[] = 'c.language IN (' . $this->_db->Quote(JFactory::getLanguage()->getTag()) . ',' . $this->_db->Quote('*') . ')';
        }
    ...
    public function getTotal() {
        if (empty($this->_total)) {
            $this->_total = MijoDB::loadRow("SELECT COUNT(DISTINCT c.id) FROM #__mijovideos_{$this->_table} AS c RIGHT JOIN #__mijovideos_videos AS v ON (v.channel_id = c.id)".$this->_buildViewWhere());
        }
    ...
    public function getItems() {
        if (empty($this->_data)) {
            ...
            foreach ($rows as $row) {
                $sql = "SELECT COUNT(channel_id) AS subs FROM #__mijovideos_subscriptions WHERE channel_id = {$row->id}";
                $this->_db->setQuery($sql);

                $row->subs = $this->_db->loadResult();

                $sql = "SELECT * FROM #__mijovideos_videos WHERE channel_id = {$row->id} LIMIT 4";
                $this->_db->setQuery($sql);
