...
<div name="adminForm" id="adminForm">
    ...
    <div class="mijovideos_box">
        ...
        <div class="mijovideos_box_content">
            ...
            <br />
            <div class="mijovideos_user_header">
                <a href="<?php echo $channel_url; ?>">
                    <img class="mijovideos_channel_thumb48" src="<?php echo $utility->getThumbPath($this->item->channel->id, 'channels', $this->item->channel->thumb); ?>"/>
                </a>
                <div class="mijovideos_channel_info">
                    <a class="mijovideos_channel_title" href="<?php echo $channel_url; ?>">
                        <?php echo $this->escape(JHtmlString::truncate($this->item->channel->title, $this->config->get('title_truncation'), false, false)); ?>
                    </a>
                    <a href="<?php echo $channel_url; ?>"><span class="mijovideos-meta-info date-created"><?php echo $this->item->channel_videos_count; ?> <?php echo JText::_('COM_MIJOVIDEOS_VIDEOS'); ?></span></a><br>
                    <?php if($this->config->get('subscriptions')) { ?>
                        <div class="mijovideos_subscribe" id="<?php echo $this->item->channel_id ?>">
                            <?php if (is_null($this->checksubscription)) { ?>
                                ...
                                <div class="mijovideos_subscribe_count" id="subs_count<?php echo $this->item->channel_id ?>"><span class="subs_count"><?php echo $this->item->channel_subs; ?></span></div>
                            ...
                            <?php } else { ?>
                                ...
                                <div style="display:none" class="mijovideos_subscribe_count" id="subs_count<?php echo $this->item->channel_id ?>"><span class="subs_count"><?php echo $this->item->channel_subs; ?></span></div>
                ...
                <div class="mijovideos_views_info">
                    <div class="mijovideos_views_count"><?php echo number_format($this->item->hits); ?> <?php echo JText::_('COM_MIJOVIDEOS_VIEWS');?></div><br>
                    <?php if ($this->config->get('likes_dislikes')) { ?>
                        <div class="views_likes_dislikes">
                            <span class="likes_count"><span id="mijovideos_like1"><?php echo $this->item->likes; ?></span> <?php echo JText::_('COM_MIJOVIDEOS_LIKES');?>&nbsp;</span>
                            <span class="dislikes_count"><span id="mijovideos_like2"><?php echo $this->item->dislikes; ?></span> <?php echo JText::_('COM_MIJOVIDEOS_DISLIKES');?></span>
                        </div>
                    <?php } ?>
                </div>
            </div>
            <?php if ($this->config->get('likes_dislikes')) { ?>
                <div class="mijovideos_actions">
                    <div class="mijovideos_like_actions">
                        <a class="<?php echo MijoVideos::getButtonClass(); ?> <?php if($this->checklikesdislikes == 1) echo 'active'; ?>" id="like_action">
                            <div class="like_button" id="like1"></div>
                            <span>&nbsp;<?php echo JText::_('COM_MIJOVIDEOS_LIKE'); ?></span>
                        </a>
                        <a class="<?php echo MijoVideos::getButtonClass(); ?> <?php if($this->checklikesdislikes == 2) echo 'active'; ?>" id="dislike_action">
                            <div class="dislike_button" id="like2"></div>
                        <span>&nbsp;<?php echo JText::_('COM_MIJOVIDEOS_DISLIKE'); ?>
                        </a>
                    </div>
                </div>
            <?php } ?>
            <?php echo JHtml::_('tabs.start', 'right', array('useCookie'=>0)); ?>
            <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_DESCRIPTION'), 'sl_desc'); ?>
            <?php echo $this->loadTemplate('description'); ?>
            <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_SHARE'), 'sl_share'); ?>
            <?php echo $this->loadTemplate('share'); ?>
            <?php if($this->config->get('playlists')) { ?>
                <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_ADDTO'), 'sl_addto'); ?>
                <?php echo $this->loadTemplate('addto'); ?>
            <?php } ?>
            <?php if ($this->config->get('reports') != "0") { ?>
                <?php echo JHtml::_('tabs.panel', JText::_('COM_MIJOVIDEOS_REPORT'), 'sl_report'); ?>
            <?php } ?>
            <?php if ($this->config->get('reports') != "0") { ?>
            ...
            <div class="mijovideos_video_comments">
                <?php
					MijoVideos::get('utility')->trigger('getComments', array($this->item->id, $this->item->title));
                ?>
            </div>
            <?php } ?>
        </div>
...
<?php if ($this->config->get('likes_dislikes')) { ?>
    <script type="text/javascript"><!--
        jQuery('#like_action,#dislike_action').click(function() {
            ...
            jQuery.ajax({
                url: 'index.php?option=com_mijovideos&view=video&task=checkLikesDislikes&format=raw&item_type=videos&item_id=<?php echo $this->item->id; ?>',
                type: 'post',
                data: {type: type},
                dataType: 'json',
                success: function(json) {
                    if (json['success']) {
                        ...
                        if(json['type'] == -1) {
                            jQuery.ajax({
                                url: 'index.php?option=com_mijovideos&view=video&task=likeDislikeItem&format=raw&item_type=videos&item_id=<?php echo $this->item->id; ?>',
                                type: 'post',
                                data: {type: type},
                                dataType: 'json',
                                success: function(json) {
