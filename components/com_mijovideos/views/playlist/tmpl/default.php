...
<div class="mijovideos_box">
    <div class="mijovideos_box_heading">
        <h1 class="mijovideos_box_h1"><?php echo $this->channelitem->title; ?></h1>
    </div>
    <div class="mijovideos_box_content">
        <!-- content -->
        <?php if ($this->channelitem->banner and file_exists(JPATH_ROOT.'/media/com_mijovideos/images/channels/' . $this->channelitem->id . '/banner/thumb/' . $this->channelitem->banner)) {
        ...
        <div class="mijovideos_channel_header">
            ...
            <?php if (MijoVideos::get('channels')->getDefaultChannel()->id == $this->channelitem->id or $this->channelitem->share_others) { ?>
                <span class="mijovideos_upload">
                    <a href="<?php echo $utility->route(JUri::base() . 'index.php?option=com_mijovideos&view=upload&channel_id='.$this->item->id.'&dashboard=1')?>" class="<?php echo MijoVideos::getButtonClass(); ?>">
                        <?php echo JText::_('COM_MIJOVIDEOS_UPLOAD'); ?>
                    </a>
                </span>
            <?php } ?>
            <?php if($this->config->get('subscriptions')) { ?>
                <div class="mijovideos_subscribe" id="<?php echo $this->channelitem->id ?>">
                    ...
                    <?php } ?>
                    <div class="mijovideos_subscribe_count" id="subs_count<?php echo $this->item->channel_id ?>"><span class="subs_count"><?php echo number_format($this->channelitem->subs); ?></span></div>
                    <div class="subs_nub"><s></s><i></i></div>
                </div>
            <?php } ?>
        </div>
        <?php echo JHtml::_('tabs.start', 'left', array('useCookie'=>1)); ?>
        <?php echo JHtml::_('tabs.panel', $this->item->title, 'sl_playlist'); ?>
		<form method="post" name="adminForm" id="adminForm" action="<?php echo JRoute::_('index.php?option=com_mijovideos&channel_id='.$this->channelitem->id.'&view=playlist'.$this->Itemid); ?>">
            ...
            <div class="mijovideos_expander" style="display: none">
            ...
            <div class="mijovideos_playlist_subheader">
                <div class="mijovideos_actions">
                    <div class="mijovideos_like_actions">
                        <a class="<?php echo MijoVideos::getButtonClass(); ?>" href="<?php echo $playlist_url; ?>">
                        ...
                        <a class="<?php echo MijoVideos::getButtonClass(); ?>" id="share_button">
...
<script type="text/javascript"><!--
    jQuery('#like_action,#dislike_action').click(function() {
        ...
        jQuery.ajax({
            url: 'index.php?option=com_mijovideos&view=playlist&task=checkLikesDislikes&format=raw&item_type=playlists&item_id=<?php echo $this->item->id; ?>',
            type: 'post',
            data: {type: type},
            dataType: 'json',
            success: function(json) {
                if (json['success']) {
                    ...
                    if(json['type'] == -1) {
                        jQuery.ajax({
                            url: 'index.php?option=com_mijovideos&view=playlist&task=likeDislikeItem&format=raw&item_type=playlists&item_id=<?php echo $this->item->id; ?>',
                            type: 'post',
                            data: {type: type},
                            dataType: 'json',
                            success: function(json) {
